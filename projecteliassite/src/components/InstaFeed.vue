<template>
  <div id="instafeed" style="width: 100%;position: relative;">
    <div style="position: absolute; right: 0; z-index: 999; height: 100%;">
      <q-btn
        @click="scrollFeed()"
        style="padding: .5rem 0; background: rgba(0, 0, 0, .5); height: 100%;"
      >
    ...
    >
      <div style="width: max-content;">
        <div
          ...
          :style="
            `height: ${$q.screen.width > 1024 ? '300' : '150'}px; width: ${
              $q.screen.width > 1024 ? '300' : '150'
        ...
        >
          <div
            ...
            :style="
              `width: 100%; padding: ${
                $q.screen.width > 1024 ? '1rem 2.5rem' : '.6rem'
              };`
            "
            @click="selectItem()"
          >
        ...
        >
          <div class="relative-position" style="height: 100%;">
            ...
            <div class="absolute" style="bottom: 0; right: 0; z-index: 999;">
              <q-btn
                style="padding: .5rem 0; background: rgba(0, 0, 0, .25);"
                @click="selectItem(item)"
              >
                <q-icon
                  :name="
                    `fas ${
                      item.media_type === 'VIDEO' ? 'fa-play' : 'fa-image'
                    }`
                  "
                  color="white"
                />
            ...
            <div
              class="centerHeaderHold"
              :style="
                `padding: ${$q.screen.width > 1024 ? '1.5rem' : '.3rem'};`
              "
            >
              <div
                :style="
                  `border: solid ${
                    $q.screen.width > 1024 ? '2' : '1'
                  }px white; height: 100%;`
                "
              >
                <div
...
export default {
  ...
  data() {
    return {
      feed: [],
      accessToken:
        "IGQVJWY19NTFZAqcDQ4M2hyZAEtUdTA0dVJvQmprRGJFbXVmTi1DRjNURlFkSWRuQ0VGMUtyTldEQ1pUOHNsdExMUFFBaU8zTjRsMEZAFOTRiWm5FUVJjMEJGSWVNMmdJTkg2b1N6WGVsRklDZAnY3NXJIek1BcGZAtNGF1bnow"
    };
  },
  ...
  methods: {
    getInstaFeed(cb) {
      this.api.get(
        `https://graph.instagram.com/me/media?fields=id,caption&access_token=${this.accessToken}`,
        res => {
    ...
    getMediaForInstaID(id, cb) {
      this.api.get(
        `https://graph.instagram.com/${id}?fields=id,media_type,media_url,username,timestamp&access_token=${this.accessToken}`,
        res => {
    ...
    hoverVideoInsta(item) {
      ...
      if (item.media_type === "VIDEO")
        document.getElementById(`instaVideo_${item.id}`).play();
    },

    leaveVideoInsta(item) {
      console.log("leaveVideoInsta: ", item);
      if (item.media_type === "VIDEO")
        document.getElementById(`instaVideo_${item.id}`).pause();
    },
...
let dummyData = [
  ...
  },
  {
    ...
    caption:
      "We hope you had a fun time with your loved ones yesterday. We've had a great year and are excited to see what 2020 brings! #throwbackthursday",
    media_type: "IMAGE",
    media_url:
